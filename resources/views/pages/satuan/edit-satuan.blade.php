...
@section('content')
    <div class="card p-4">
        ...
        <h3 class="text-black fw-bold">Edit Satuan</h3>
        <form action="{{ route('satuan.update', $satuan->id) }}" method="post">
            @csrf
            @method('put')
            <label for="">Satuan</label>
            <input type="text" name="nama" class="form-control mb-3" value="{{ $satuan->nama }}">
            <label for="">Satuan Kecil</label>
            <select class="form-select mb-3" name="reference_satuan_id">
                <option value="">Pilih</option>
                @foreach ($satuans as $item)
                <option value="{{ $item->id }}"
                    {{ $item->id == ($satuan->reference_satuan_id ?? '') ? 'selected' : '' }}>
                    {{ $item->nama }}
                </option>
                @endforeach
            </select>
            <label for="">Nilai</label>
            <input type="hidden" name="nilai" id="nilai_hidden" value="{{ $satuan->nilai }}">
            <input type="text" inputmode="numeric" class="form-control mb-3 nilai" value="{{ $satuan->nilai }}" min="0">
            <div class="text-end">
